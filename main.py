...
# x_points = np.arange(-20, 20, 4, dtype = float).reshape((10, 1))
# y_points = np.zeros((10, 1), dtype = float)
# z_points = np.zeros((10, 1), dtype = float)

x_points = np.array([[0.1]], dtype = float)
y_points = np.array([[0]], dtype = float)
z_points = np.array([[0]], dtype = float)
...
def update(frame):
    ...
    if(TRANSFORM_TYPE == 0):
        dx, dy, dz = lorenzTransform(x, y, z)

        ax.axes.set_xlim3d(left = -25, right = 25) 
        ax.axes.set_ylim3d(bottom = -25, top = 25) 
        ax.axes.set_zlim3d(bottom = -25, top = 25)
    elif(TRANSFORM_TYPE == 1):
        dx, dy, dz = aizawaTransform(x, y, z)

        ax.axes.set_xlim3d(left = -2, right = 2) 
        ax.axes.set_ylim3d(bottom = -2, top = 2) 
        ax.axes.set_zlim3d(bottom = -2, top = 2)
    elif(TRANSFORM_TYPE == 2):
        dx, dy, dz = halvorsenTransform(x, y, z)

        ax.axes.set_xlim3d(left = -50, right = 50) 
        ax.axes.set_ylim3d(bottom = -50, top = 50) 
        ax.axes.set_zlim3d(bottom = -50, top = 50)
    elif(TRANSFORM_TYPE == 3):
        dx, dy, dz = financeTransform(x, y, z)

        ax.axes.set_xlim3d(left = -5, right = 5) 
        ax.axes.set_ylim3d(bottom = -5, top = 5) 
        ax.axes.set_zlim3d(bottom = -5, top = 5)
    elif(TRANSFORM_TYPE == 4):
        dx, dy, dz = nosehooverTransform(x, y, z)
        ...
        for i in range(x.shape[0]):
            for j in range(len(COLORS)):
                if(i >= j * x.shape[0] / len(COLORS) and i < (j + 1) * x.shape[0] / len(COLORS)):
                    ax.plot(x_points[i], y_points[i], z_points[i], color = COLORS[j])
                    break
